...
public class ReadXMLFile {
	...
	public HashMap<String, String >  getSubmittedOptyFromXML()throws ParserConfigurationException, SAXException, IOException{
		ConfigReader config = new ConfigReader();
		Properties prop = config.init_properties();
		...
		for (int i = 0; i < nodeList.getLength(); i++)
		{
			...
			if (node.getNodeType() == Node.ELEMENT_NODE)
			{
				...
				OptyHeader.put("OPPORTUNITY_NAME",eElement.getElementsByTagName("OPPORTUNITY_NAME").item(0).getTextContent());
				...
				OptyHeader.put("MARKET_SEGMENT",eElement.getElementsByTagName("MARKET_SEGMENT").item(0).getTextContent());
				OptyHeader.put("MARKETER_NAME",eElement.getElementsByTagName("MARKETER_NAME").item(0).getTextContent());
