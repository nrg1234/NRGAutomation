...
public class AddCustomerPopup {
	...
	WebElement savebtn;	
	//div[@class='control__indicator site-checkbox']
	@FindAll(value = { @FindBy(xpath = "//div[@class='control__indicator site-checkbox']") })
	public List<WebElement> Sitecbox;
...
//	WebElement Seibelrecord;
	...
	@FindBy(xpath = "//label[contains(text(), 'CMA Effective Date')]")
	WebElement CMAfield;
...
//		if(customeraddresssource.get("Siebel"))
//		functions.click(driver, firstCustomerCard);
		try {
if(CMAfield.isDisplayed()) {
	System.out.println("CMA field present");
...
//		 functions.minwait();
	...
	public void ViewHeader(String opportunityname) throws Throwable {
		CommonFunctions functions = new CommonFunctions();
		
		//Click on seedetails
		functions.click(driver, seedetailsbtn);
		functions.minwait();
		 String alpha ="NewAnalystoptyheader";
	...
	public String SelectRenewButton(String renewCust) throws Throwable {
		...
		Sitecbox.get(0).click();
		functions.shortWait();
		Sitecbox.get(1).click();
...
//		functions.click(driver, SaveButton);
	...
	public String SelectRenewOption() throws Throwable {
		...
		if (contractCheckbox50kwh.size() > 0 == true && contractCheckbox50kwh.get(1).isDisplayed() == true) {
			contractCheckbox50kwh.get(0).click();
		}
		// Wait for sometime
		functions.shortWait();
...
//		return contracttext;
		...
		WebDriverWait wait1 = new WebDriverWait(driver, 200);
		wait1.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//input[@id='typeahead-customer']")));
		functions.setText(driver, customerNameInput, prop.getProperty("Renewallesscustomername"));
...
//		Sitecbox.get(1).click();
		...
		WebDriverWait wait91 = new WebDriverWait(driver, 200);
		wait91.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//input[@id='typeahead-customer']")));
		functions.setText(driver, customerNameInput, prop.getProperty("RenewalLessAggcustomer"));
