...
public class MeterFeeandVoluntaryRecs {
	...
	public void MeterFeeandVoluntaryRecs() throws Throwable {
		...
		//Entering MVP Value
				functions.selectDropdownByVisibleText(driver, MVPDropdown, "35%");
	...
	public Boolean ValidateMVP() throws Throwable {
		CommonFunctions functions = new CommonFunctions();
		// Navigate to quotes page
		WebDriverWait wait1 = new WebDriverWait(driver, 400);
		wait1.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//button[contains(text(), 'Quotes')]")));
		...
		functions.shortWait();
	     ...
	     //Validating default MVP Value
		Select select=new Select(MVPDropdown);
		List<WebElement> drop=select.getOptions();
		

		for(WebElement element:drop) {
			
			if(element.getText().equals("25%")&& element.getText().equals("35%")) {
