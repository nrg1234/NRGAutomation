...
public class CreateOptyPopup {
  ...
  //label[contains(text(),'New Analyst')]
    @FindBy(xpath = "//label[contains(text(),'New Analyst')]")
    WebElement NewAnalyst;
...
//        functions.shortWait();
    ...
    public String CreateNewOpportunityReliant(HashMap<String, String> OptyHeader) throws Throwable {
        CommonFunctions functions = new CommonFunctions();
        Actions action = new Actions(driver);
        ConfigReader config = new ConfigReader();
        Properties prop = config.init_properties();
...
// select brand
    ...
    public String CreateMatrixlessthan50Opportunity(HashMap<String, String> OptyHeader) throws Throwable {
        CommonFunctions functions = new CommonFunctions();
        ConfigReader config = new ConfigReader();
        Actions action = new Actions(driver);
        Properties prop = config.init_properties();
...
//        functions.setText(driver, endDate, OptyHeader.get("END_DATE"));
    ...
    public String CreateAnalyst(HashMap<String, String> OptyHeader) throws Throwable {
        CommonFunctions functions = new CommonFunctions();
        ConfigReader config = new ConfigReader();
        Actions action = new Actions(driver);
        Properties prop = config.init_properties();
...
//        functions.setText(driver, endDate, OptyHeader.get("END_DATE"));
      ...
      //  functions.selectDropdownByVisibleText(driver, marketsegment, prop.getProperty("SalesGroup"));
       ...
       // functions.selectDropdownByVisibleText(driver, contractbrand, prop.getProperty("ContractBrand"));
		...
		js.executeScript("arguments[0].scrollIntoView();", NewAnalyst);
        String alpha ="NewAnalyst";
        functions.takeSnapShot(driver, Opportunityname,alpha);
        // Click "create Button"
        List<WebElement> Createbutton = createBtn;
        if (Createbutton.size() > 0 == true && Createbutton.get(1).isDisplayed() == true) {
